...
import { renderPageThumbnail, getPdfPageCount } from "@/lib/tools/pdf-splitter";
import type { PageConfig } from "@/lib/tools/pdf-page-utils";
...
// ─── Main PdfPageManager Component ────────────────────────────────

interface PdfPageManagerProps {
  /** The PDF file to manage pages for */
  file: File;
  /** Called when user confirms page configuration */
  onConfirm: (pages: PageConfig[]) => void;
  /** Called when user cancels / wants to go back */
  onCancel?: () => void;
  /** Label for the confirm button (default: "Download PDF") */
  confirmLabel?: string;
  /** Label for the cancel button (default: "Cancel") */
  cancelLabel?: string;
  /** Whether to disable confirm when no changes made (default: true) */
...
export function PdfPageManager({
    ...
    (async () => {
      ...
      try {
        ...
        for (let i = 0; i < count; i++) {
          if (cancelled) break;
          try {
            const url = await renderPageThumbnail(file, i, 150);
            if (!cancelled) {
  ...
  const rotatePage = useCallback((pageIndex: number, deg: number) => {
    setRotations((prev) => {
      const next = new Map(prev);
      const current = next.get(pageIndex) || 0;
      const newAngle = normalizeAngle(current + deg);
      if (newAngle === 0) {
  ...
  const removePg = useCallback((posIdx: number) => {
    setPageOrder((prev) => {
  ...
  const handleConfirm = useCallback(() => {
    const allPages: PageConfig[] = Array.from({ length: pageCount }, (_, i) => ({
      originalIndex: i,
      rotation: rotations.get(i) || 0,
      included: false,
    }));

    // Mark included pages in order
    for (const pageIdx of pageOrder) {
      allPages[pageIdx].included = true;
    }

    // Build ordered result: included pages in current order, then excluded
    const ordered: PageConfig[] = pageOrder.map((idx) => allPages[idx]);
    const excluded = allPages.filter((p) => !p.included);
    onConfirm([...ordered, ...excluded]);
  }, [pageCount, pageOrder, rotations, onConfirm]);
