...
const PRESET_3X4 = PHOTO_PRESETS[1]; // 3×4 cm
const PRESET_4X6 = PHOTO_PRESETS[2]; // 4×6 cm
...
function makeCustomPreset(w: number, h: number): PhotoPreset {
  return {
    id: "custom",
    label: `${w}\u00d7${h}`,
    widthCm: w,
...
export default function ResizeImagePage() {
  ...
  let frameW = Math.round(selectedPreset.widthCm * FRAME_PX_PER_CM);
  let frameH = Math.round(selectedPreset.heightCm * FRAME_PX_PER_CM);
  if (frameH > FRAME_MAX_H) {
    const r = FRAME_MAX_H / frameH;
    frameH = FRAME_MAX_H;
    frameW = Math.round(frameW * r);
  }
  if (frameW > FRAME_MAX_W) {
    const r = FRAME_MAX_W / frameW;
    frameW = FRAME_MAX_W;
    frameH = Math.round(frameH * r);
  }
  frameW = Math.max(frameW, 80);
  frameH = Math.max(frameH, 80);
  ...
  function handlePresetClick(preset: PhotoPreset) {
  ...
  const handleRotateCW = () =>
    applyTransform({ rotation: 90, flipHorizontal: false, flipVertical: false });
  const handleRotateCCW = () =>
    applyTransform({ rotation: 270, flipHorizontal: false, flipVertical: false });
  const handleFlipH = () =>
    applyTransform({ rotation: 0, flipHorizontal: true, flipVertical: false });
  const handleFlipV = () =>
    applyTransform({ rotation: 0, flipHorizontal: false, flipVertical: true });
  ...
  async function handleRemoveBg() {
    ...
    setShowBgPanel(true);
    try {
      setPreBgImage({ blob: imageBlob, url: imageUrl, dims: { ...imageDims } });
      ...
      setBgRemoved(true);
      const comp = await addColorBackground(res.blob, bgColor, res.width, res.height);
  ...
  async function handleBgColorSelect(color: string) {
    ...
    setCompositing(true);
    try {
      const comp = await addColorBackground(foregroundBlob, color, imageDims.width, imageDims.height);
      updateWorkingImage(comp.blob, comp.previewUrl, imageDims.width, imageDims.height);
    } finally {
      setCompositing(false);
    }
  }

  async function handleBgImageSelected(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setCompositing(true);
    try {
      const comp = await addImageBackground(foregroundBlob, file, imageDims.width, imageDims.height, { scale: 100, position: "center" });
      updateWorkingImage(comp.blob, comp.previewUrl, imageDims.width, imageDims.height);
    } finally {
      setCompositing(false);
    }
  ...
  useEffect(() => {
    ...
    compositeTimerRef.current = setTimeout(async () => {
      setCompositing(true);
      try {
        const comp = await addImageBackground(foregroundBlob, bgImageFile, imageDims.width, imageDims.height, { scale: bgScale, position: bgPosition });
        updateWorkingImage(comp.blob, comp.previewUrl, imageDims.width, imageDims.height);
      } finally {
        setCompositing(false);
      }
  ...
  function handleResetBg() {
    ...
    updateWorkingImage(preBgImage.blob, preBgImage.url, preBgImage.dims.width, preBgImage.dims.height);
    setBgRemoved(false);
  ...
  async function handleCropApply(cropArea: CropArea, rotation: 0 | 90 | 180 | 270) {
    if (!imageBlob) return;
    setCropLoading(true);
    try {
      const res = await cropImage(toFile(imageBlob), { cropArea, rotation });
      updateWorkingImage(res.blob, res.previewUrl, res.croppedWidth, res.croppedHeight);
      if (foregroundBlob) {
        const fgRes = await cropImage(toFile(foregroundBlob, "fg.png"), { cropArea, rotation });
        setForegroundBlob(fgRes.blob);
      }
      resetViewport();
      setStage("editor");
  ...
  async function handleProcess() {
    ...
    try {
      ...
      const res = await resizeImage(imageBlob, selectedPreset, cropRect, (u) => setProgress(u));
      setResult(res);
      setStage("done");
    } catch (err) {
  ...
  function handleDownload() {
    ...
    const fileName = `${baseName}_${selectedPreset.id}_${result.resizedWidth}x${result.resizedHeight}.jpg`;
    ...
    <ToolPageLayout tool={tool}>
      ...
      {stage === "upload" && (
        <FileUploader
          ...
          subtitle="Resize photos to standard formal sizes for documents, passports, and visas"
        />
      )}
      ...
      {stage === "editor" && (
        <div className="space-y-6">
          {/* Size selection */}
          <div>
            <h3 className="text-sm font-medium text-slate-700 mb-2">Photo Size</h3>

            {/* Preset buttons */}
            <div className="flex flex-wrap gap-2 mb-3">
              <button
                onClick={() => handlePresetClick(PRESET_3X4)}
                className={`px-4 py-2.5 rounded-lg border text-sm font-semibold transition-all ${
                  selectedPreset.id === "3x4"
              ...
              <button
                onClick={() => handlePresetClick(PRESET_4X6)}
                className={`px-4 py-2.5 rounded-lg border text-sm font-semibold transition-all ${
                  selectedPreset.id === "4x6"
                    ? "border-amber-500 bg-amber-50 text-amber-800 ring-1 ring-amber-500"
                    : "border-slate-200 bg-white text-slate-700 hover:border-slate-300"
                }`}
              >
            ...
            <p className="text-xs text-slate-400 mt-2">
              {selectedPreset.widthPx}{"\u00d7"}{selectedPreset.heightPx} px {"\u00b7"} {PRINT_DPI} DPI
            </p>
          </div>
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            {isLowRes && (
              <div className="flex items-center gap-2 px-3 py-2 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-700">
                ...
                Image resolution may be too low for optimal print quality at this size
              </div>
            )}
          </div>
          ...
          <div className="space-y-4 border-t border-slate-100 pt-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <span className="text-sm font-medium text-slate-700 mr-1">Adjust</span>
              <button onClick={handleRotateCCW} disabled={rotating} className="p-2 rounded-lg border border-slate-200 hover:bg-slate-50 disabled:opacity-40" title="Rotate counterclockwise">
                <RotateCcw className="w-4 h-4" />
              </button>
              <button onClick={handleRotateCW} disabled={rotating} className="p-2 rounded-lg border border-slate-200 hover:bg-slate-50 disabled:opacity-40" title="Rotate clockwise">
                <RotateCw className="w-4 h-4" />
              </button>
              <button onClick={handleFlipH} disabled={rotating} className="p-2 rounded-lg border border-slate-200 hover:bg-slate-50 disabled:opacity-40" title="Flip horizontal">
                <FlipHorizontal2 className="w-4 h-4" />
              </button>
              <button onClick={handleFlipV} disabled={rotating} className="p-2 rounded-lg border border-slate-200 hover:bg-slate-50 disabled:opacity-40" title="Flip vertical">
                <FlipVertical2 className="w-4 h-4" />
              </button>
              <div className="w-px h-6 bg-slate-200 mx-1" />
              <button onClick={() => setStage("crop")} className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-slate-200 hover:bg-slate-50 text-sm">
            ...
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                {!bgRemoved && !bgRemoving && (
                  <button onClick={handleRemoveBg} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-800 text-white text-sm hover:bg-slate-700 transition">
                    <Wand2 className="w-3.5 h-3.5" />
                    Remove Background
                  </button>
                )}
                {bgRemoved && (
                  <button onClick={handleResetBg} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-200 text-sm hover:bg-slate-50">
              ...
              {bgRemoving && (
                <div className="flex items-center gap-3 px-3 py-2 bg-slate-50 rounded-lg">
                  ...
                  <div className="flex-1">
                    <div className="text-xs text-slate-600 mb-1">{bgProgress.stage}</div>
                    <div className="h-1.5 bg-slate-200 rounded-full overflow-hidden">
                      <div className="h-full bg-amber-500 transition-all duration-200 rounded-full" style={{ width: `${bgProgress.progress}%` }} />
                    </div>
                  </div>
                </div>
              )}
              ...
              {bgRemoved && !bgRemoving && (
                <div className="space-y-3">
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      <button onClick={() => setBgMode("color")} className={`text-xs px-2 py-1 rounded ${bgMode === "color" ? "bg-amber-100 text-amber-800" : "bg-slate-100 text-slate-600"}`}>Color</button>
                      <button onClick={() => setBgMode("image")} className={`text-xs px-2 py-1 rounded ${bgMode === "image" ? "bg-amber-100 text-amber-800" : "bg-slate-100 text-slate-600"}`}>Image</button>
                    </div>
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        {BG_COLORS.map(({ color, label }) => (
                          <button
                            ...
                            className={`w-8 h-8 rounded-full border-2 transition-all ${bgColor === color && bgMode === "color" ? "border-amber-500 ring-2 ring-amber-200 scale-110" : "border-slate-300 hover:scale-105"}`}
                            style={{ backgroundColor: color }}
                            title={label}
                          />
                        ))}
                        <label className="relative">
                          <input type="color" value={bgColor} onChange={(e) => handleBgColorSelect(e.target.value)} className="absolute inset-0 opacity-0 cursor-pointer w-8 h-8" />
                        ...
                        {compositing && <Loader2 className="w-4 h-4 animate-spin text-slate-400" />}
                      </div>
                    )}
                    ...
                    {bgMode === "image" && (
                      <div className="space-y-2">
                        <label className="flex items-center gap-2 px-3 py-2 border border-dashed border-slate-300 rounded-lg cursor-pointer hover:border-slate-400 text-sm text-slate-600">
                          ...
                          <input type="file" accept=".jpg,.jpeg,.png" onChange={handleBgImageSelected} className="hidden" />
                        </label>
                        {bgImageFile && (
                          <>
                            <div className="flex items-center gap-2">
                              <span className="text-xs text-slate-500 w-10">Scale</span>
                              <input type="range" min={10} max={300} value={bgScale} onChange={(e) => setBgScale(parseInt(e.target.value))} className="flex-1 accent-amber-500" />
                              <span className="text-xs font-medium text-slate-600 w-10 text-right">{bgScale}%</span>
                            </div>
                            <div className="flex items-center gap-2">
                              <span className="text-xs text-slate-500 w-10">Position</span>
                              <div className="grid grid-cols-3 gap-1">
                                {POSITION_GRID.map(({ pos }) => (
                                  <button key={pos} onClick={() => setBgPosition(pos)} className={`w-5 h-5 rounded-sm border transition-all ${bgPosition === pos ? "bg-amber-500 border-amber-500" : "bg-slate-100 border-slate-300 hover:bg-slate-200"}`} />
                                ))}
                              </div>
                              {compositing && <Loader2 className="w-4 h-4 animate-spin text-slate-400" />}
                            </div>
      ...
      {stage === "processing" && (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          ...
          <div className="w-64 h-2 bg-slate-200 rounded-full overflow-hidden">
            <div className="h-full bg-amber-500 transition-all duration-200 rounded-full" style={{ width: `${progress.progress}%` }} />
          </div>
        </div>
      )}
      ...
      {stage === "done" && result && (
        <div className="space-y-6">
          <div className="text-center">
            ...
            <h2 className="text-xl font-semibold text-slate-800">Resize Complete!</h2>
          </div>

          <div className="flex justify-center">
            <img src={result.previewUrl} alt="Resized" className="max-h-64 border border-slate-200 rounded-lg shadow-sm" />
          </div>
          ...
          <div className="bg-slate-50 rounded-xl p-4 space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className="font-medium text-slate-700">
                {result.preset.widthCm}{"\u00d7"}{result.preset.heightCm} cm
              </span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium text-slate-700">{formatSize(result.resizedSize)}</span>
            </div>
            <div className="flex justify-between">
          ...
          <div className="flex items-center gap-2 px-3 py-2 bg-emerald-50 border border-emerald-200 rounded-lg text-sm text-emerald-700">
            ...
            {"Print-ready at "}{PRINT_DPI}{" DPI \u00b7 Optimal quality"}
          </div>

          <div className="px-3 py-2 bg-blue-50 border border-blue-200 rounded-lg text-sm text-blue-700">
            Output is JPEG at maximum quality with {PRINT_DPI} DPI metadata embedded. Ready for professional printing.
          </div>

          <div className="flex flex-col sm:flex-row gap-2">
            <button onClick={handleDownload} className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-amber-500 text-white font-semibold hover:bg-amber-600 transition">
              <Download className="w-5 h-5" />
              Download
            </button>
            <button onClick={handleBackToEdit} className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl border border-slate-200 text-slate-700 font-medium hover:bg-slate-50 transition">
              <ArrowLeft className="w-5 h-5" />
              Back to Edit
            </button>
            <button onClick={handleProcessAnother} className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl border border-slate-200 text-slate-700 font-medium hover:bg-slate-50 transition">
