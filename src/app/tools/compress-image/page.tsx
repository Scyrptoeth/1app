...
const MODE_ICONS: Record<ImageCompressionMode["id"], { bars: number; color: string }> = {
...
export default function CompressImagePage() {
  ...
  const handleModeSelect = useCallback(
    async (mode: ImageCompressionMode) => {
      if (!file) return;
      setStage("processing");

      try {
        const compressionResult = await compressImage(file, mode, (update) =>
          setProgress(update)
        );
        ...
        setStage("mode-select");
        alert(
          "Failed to compress the image. The file may be corrupted or too large for the browser to process. Please try a different file."
        );
      }
    },
    [file]
    ...
    <ToolPageLayout tool={tool}>
      ...
      {stage === "upload" && (
        <FileUploader
          ...
          subtitle="Supports JPEG and PNG images up to 100MB"
        />
      )}
      ...
      {stage === "mode-select" && file && (
        <div className="w-full max-w-3xl mx-auto">
          ...
          <p className="text-sm text-slate-500 mb-6">
            Select a mode based on your quality and size requirements. Dimensions stay the same.
          </p>
          ...
          {result.compressionRatio <= 0 && (
            <div className="mb-6 flex items-start gap-3 p-3 bg-amber-50 border border-amber-100 rounded-xl">
              ...
              <p className="text-xs text-amber-700 leading-relaxed">
                This image is already well-optimized. Try a higher compression
