...
import {
  ...
  type ReorderPdfResult,
  type PageDimensions,
} from "@/lib/tools/pdf-reorder";
...
export default function ReorderPdfPage() {
  ...
  const [result, setResult] = useState<ReorderPdfResult | null>(null);
  ...
  const isDefaultOrder = activePages.length === pageCount
  ...
  const handleProcess = useCallback(async () => {
    ...
    try {
      const processResult = await reorderPdf(file, activePages, (update) => setProgress(update), rotations);
          ...
          <div className="pt-4 border-t border-slate-100">
            <div className="flex items-center justify-between mb-3">
              <div className="text-xs text-slate-500">
                ...
                {hasChanges && !isDefaultOrder && removedPages.length === 0 && (
                  <span className="text-slate-400"> &middot; reordered</span>
                )}
              </div>
            ...
            {activePages.length === 0 && (
              <p className="mt-2 text-xs text-amber-600 text-center">
                At least 1 page must remain to download.
              </p>
            )}
          </div>
        </div>
      ...
      {stage === "done" && result && (
        <>
          <div className="w-full max-w-lg mx-auto text-center">
            ...
            <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-xl mb-6">
              ...
              <div className="text-left min-w-0">
                ...
                <p className="text-xs text-slate-500">{formatFileSize(result.reorderedSize)}</p>
              </div>
            </div>
          ...
          <div className="mt-6 flex items-center justify-center gap-6 text-sm text-slate-500">
            ...
            <div className="text-slate-300">|</div>
            <div className="flex items-center gap-1.5">
              <span>{formatFileSize(result.originalSize)} &rarr; {formatFileSize(result.reorderedSize)}</span>
            </div>
          </div>
